...
def model_dirs() -> list[Path]:
    ...
    try:
        ...
        pass
    root_models = Path(__file__).resolve().parent.parent / "models" / MODEL_FOLDER_NAME
    local_models = Path(__file__).resolve().parent / "models" / MODEL_FOLDER_NAME
    return [root_models, local_models]
...
def resolve_device(device_name: str) -> torch.device:
    ...
    if device.type == "cuda":
        ...
        if device.index is None:
            ...
            # an explicit index, so pin the current device (GitHub issue #1).
