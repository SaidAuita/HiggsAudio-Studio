...
* [Higgs Audio Studio](#higgs-audio-studio)
  ...
  + [Системные требования](#системные-требования)
    - [Платформы (через Pinokio-лаунчер)](#платформы-через-pinokio-лаунчер)
    - [Память (NVIDIA; TTS квантуется на лету, LLM-режиссёр грузится отдельно)](#память-nvidia-tts-квантуется-на-лету-llm-режиссёр-грузится-отдельно)
  + [Быстрый старт](#быстрый-старт)
  + [Другие проекты @timoncool](#другие-проекты-timoncool)
  + [Авторы](#авторы)
  + [Благодарности](#благодарности)
  + [Лицензия](#лицензия)
  + [Обновление 3.1](#обновление-31)
    - [Добавлено в этом форке:](#добавлено-в-этом-форке)
    - [Быстрый старт версии 3.1:](#быстрый-старт-версии-31)
  + [Поддержка этого форка](#поддержка-этого-форка)
<!-- /super-markdown-toc -->
...
## Обновления режима «Длинный клон»

- **Дополнительная верхняя кнопка «Озвучить»**: добавлена кнопка синтеза прямо над текстовым окном для быстрой работы без прокрутки вниз.
- **Сквозная кастомная нумерация (Кастомный номер)**:
  - Автоматическое сохранение всех склеенных фрагментов в общую папку `output/NUM/<номер>.<формат>`.
  - Динамическое сохранение разрядности (например: `01` -> `02`, `005` -> `006`).
  - Проверка существования файла и выдача предупреждений `gr.Warning()` перед началом генерации.
  - Автоматическое увеличение номера на +1 при успешном завершении синтеза.
- **Оптимизированный интерфейс**:
  - Чекбокс «Кастомный номер» и ввод номера объединены в компактный однострочный блок одинаковой высоты со всеми контролами.
  - Убраны синие встроенные оверлей-бары индикации прогресса над текстовыми инпутами, лог выводится построчно в поле «Лог генерации».
